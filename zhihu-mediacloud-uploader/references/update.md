...
用于判断当前安装的 `zhihu-mediacloud-uploader` 是否落后于版本接口发布的版本，并在用户确认后更新。
...
- 每个会话首次加载 `zhihu-mediacloud-uploader` skill 后，自动检查一次。
...
| `repository` | 首选更新方式使用的仓库标识，例如 `zhihu/zhihu-mediacloud-uploader` |
...
```bash
mediacloud_version_base_url="${ZHIHU_MEDIACLOUD_BASE_URL:-https://openapi.zhihu.com}"
curl --fail --silent --show-error "${mediacloud_version_base_url%/}/openapi/publish_skill/version"
```

Windows（PowerShell）：

```powershell
$mediacloudVersionBaseUrl = if ([string]::IsNullOrWhiteSpace($env:ZHIHU_MEDIACLOUD_BASE_URL)) {
  "https://openapi.zhihu.com"
} else {
  ($env:ZHIHU_MEDIACLOUD_BASE_URL).TrimEnd("/")
}
curl.exe --fail --silent --show-error "$mediacloudVersionBaseUrl/openapi/publish_skill/version"
```

Windows 必须显式调用 `curl.exe`，不要调用可能映射到 `Invoke-WebRequest` 的 PowerShell `curl` 别名。

1. 结构化解析 response JSON，不要用字符串截取字段。
2. 使用本地 `SKILL.md` 的 `name` 在顶层对象中做精确匹配；本 skill 只读取 `zhihu-mediacloud-uploader` 对象，不读取其他 skill 的版本。
3. 读取匹配对象的 `version`、`repository` 和 `download_link`；不要自行推导或替换这些值。
4. 按语义化版本的数字顺序比较接口 `version` 与本地 `metadata.version` 的 `MAJOR.MINOR.PATCH`。
...
检查发现新版本时，先提示用户更新并等待选择；用户选择前不要继续上传或初始化流程。用户选择暂不更新时，可以继续后续流程，但必须说明当前会话仍使用本地已加载版本。
...
将 `{repository}` 替换为接口返回的原始值。命令成功后，提示用户完全退出并重启 Agent；重启前不要继续上传或初始化流程。
...
```bash
curl --fail --location --output zhihu-mediacloud-uploader-{version}.zip "{download_link}"
```

Windows（PowerShell）：

```powershell
curl.exe --fail --location --output "zhihu-mediacloud-uploader-{version}.zip" "{download_link}"
```
...
5. 提示用户完成拷贝后完全退出并重启 Agent；重启前不要继续上传或初始化流程。
